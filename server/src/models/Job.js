...
const JobModel = {
    ...
    async create({ customer_id, device_id, technician_id, created_by, device_name, problem_description, priority, estimated_cost }) {
        const result = await runQuery(
            `INSERT INTO jobs (customer_id, device_id, technician_id, created_by, device_name, problem_description, priority, estimated_cost) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [customer_id, device_id || null, technician_id || null, created_by, device_name, problem_description, priority || 'medium', estimated_cost || 0]
        );
    ...
    async getStats() {
        ...
        const [customersRow, devicesRow, jobsRow, revenueRow] = await Promise.all([
            db.queryOne('SELECT COUNT(*) as c FROM customers'),
            db.queryOne('SELECT COUNT(*) as c FROM devices'),
            db.queryOne(`
                SELECT 
    ...
    async getMonthlyStats(year, month) {
        ...
        const topDeviceRow = await queryOne(`SELECT d.brand, COUNT(j.id) as count FROM jobs j JOIN devices d ON j.device_id = d.id WHERE DATE_FORMAT(j.date_in, '%Y-%m') = ? GROUP BY d.brand ORDER BY count DESC LIMIT 1`, [monthStr]);
        ...
        return {
            ...
            top_device: topDeviceRow ? `${topDeviceRow.brand} (${topDeviceRow.count})` : 'N/A',
